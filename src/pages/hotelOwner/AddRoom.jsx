...
const AddRoom = () => {
  ...
  return (
    <form > 
        <Title align='left' font='outfit' title='Add Room'subTitle='Fill carefully the accurate and correct details, pricing and amenities to enhance booking experience of user.' />
        {/* Upload images */}
        ...
        <div className='w-full flex max-sm:flex-col sm:gap-4 mt-6'>
            <div className='flex-1 max-w-48'>
                ...
                className="border opacity-70 border-gray-300 mt-1 rounded p-2 w-full">
                    ...
                    <option value="Luzury Room">Luzury Room</option>
                </select>
            </div>
            <div className='mt-4 text-gray-800 '>
                <p>Price <span className='text-xs'>/night</span></p>
                <input type="number" placeholder="0" className="border border-gray-300 mt-1 rounded p-2 w-36" value={inputs.pricePerNight} onChange={e=>setInputs({...inputs, pricePerNight:e.target.value})} />
            </div>
        </div>
        {/* Amenities */}
          ...
          <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
            {Object.keys(inputs.amenities).map((amenity, index) => (
                <div key={index} className='flex gap-3'>
                    ...
                    <label htmlFor={`amenities${index + 1}`} className='text-gray-700'>{amenity}</label>
                </div>
            ))}
        </div>
